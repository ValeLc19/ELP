...
You'll get a URL like `https://elpaso-events.pages.dev`. Every run redeploys.
...
1. **Backend CORS:** set `CORS_ORIGINS` on Render to your Pages URL
   (e.g. `https://elpaso-events.pages.dev`) so the browser can call the API.
2. **Supabase auth URLs:** Authentication → URL Configuration →
   - **Site URL**: your Pages URL
   - **Redirect URLs**: add `https://elpaso-events.pages.dev/**` and keep
